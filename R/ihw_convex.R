...
ihw.default <- function(pvalues, covariates, alpha,
                        ...
                        lp_solver = "lpsymphony", # lpsymphony
                        adjustment_type = "BH",
                        ...
                        strat = "quantile",
                        ntrees = 10L,
                        taus = NULL, ntaus =5, maxdepth = NULL, min.node.size = NULL,
                        nbasis = 5L,
                        basis_system ="Legendre",
                        ...) {
  ...
  if (covariate_type == "ordinal" & is.numeric(covariates)) {
    ...
    if(strat == "quantile"){
      groups <- groups_by_filter(covariates, nbins, seed = seed)
      groups <- as.data.frame(groups)
    }else if(strat == "cut"){
      groups <- groups_by_cut(covariates, nbins)
      groups <- as.data.frame(groups)
    }else if(strat %in% c("BocaLeek","RFCDE", "Julia")){
      if (is.null(folds)) { # TODO this is the spot
        if (!is.null(seed)) {
        ...
        folds <- sample(1:nfolds, m_with_na, replace = TRUE)
      }
      
      if(strat == "BocaLeek"){
        groups <- group_by_forest_BocaLeek(pvalues, covariates, folds[nna], nbins=nbins, ntrees = ntrees, taus = taus, ntaus = ntaus, maxdepth = maxdepth, min.node.size = min.node.size) 
      }else if(strat == "RFCDE"){
        groups <- group_by_forest_RFCDE(pvalues, covariates, folds[nna], nbins = nbins, ntrees = ntrees, nbasis = nbasis, basis_system = basis_system)
      }else if(strat == "Julia"){
        groups <- group_by_forest_Julia(pvalues, covariates, folds[nna], nbins=nbins, ntrees = ntrees, nbasis = nbasis)
      }
    }else{
      print("unknown stratification method")
    }
  ...
  group_levels <- lapply(groups, function(groups_i) levels(droplevels(groups_i)))
  ...
  sorted_groups <- groups[order_pvalues, , drop = F]
  #sorted_groups <- lapply(groups, function(groups_i) {
  #  col <- groups_i[order_pvalues, drop = F]
  #  as.factor(col)
  #})
  #sorted_groups <- as.data.frame(sorted_groups)
  ...
  if (is.null(m_groups)) {
    if (is.null(folds)) {
      m_groups <- lapply(sorted_groups, function(sorted_groups_i) table(sorted_groups_i))
    } else {
      m_groups <- lapply(sorted_groups, function(sorted_groups_i) table(sorted_groups_i, sorted_folds))
    }
  } else {
  ...
  if (is.null(folds)) {
    incorrect_levels <- mapply(function(group_levels_i, m_groups_i){
      length(group_levels_i) != length(m_groups_i)
    }, group_levels, m_groups)

    if (any(incorrect_levels)) { 
      stop("Number of unique levels should be equal to length of m_groups.")
    }
  } else {
    correct_levels <- sapply(m_groups, function(m_groups_i) any(dim(m_groups) != c(length(group_levels), nfolds)))
    if (any(correct_levels)) {
      stop("m_groups should be of dimension nbins*nfolds")
    }
  }
  # once we have groups, check whether they include enough p-values
  not_enough_p_values <- sapply(m_groups, function(m_groups_i) any(m_groups_i < 1000))
  if (any(nbins > 1) & any(not_enough_p_values)) {
  ...
  if (all(nbins == 1)) { #TODO
    ...
    res <- list(
      lambda = 0, fold_lambdas = 0, rjs = rjs, sorted_pvalues = sorted_pvalues,
    ...
    #res <- lapply(seq_along(sorted_groups), function(i){ #TODO
  #    sorted_groups_i <- sorted_groups[[i]]
  #    m_groups_i <- m_groups[[i]]
  ...
  #groups <- as.data.frame(sorted_groups)
  #groups[] <- lapply(seq_along(group_levels), function(t){
  #groups <- lapply(seq_along(group_levels), function(t){ #TODO
    #factor(fill_nas_reorder(res$sorted_groups[[t]], nna, reorder_pvalues), levels = group_levels[[t]])
  #  fill_nas_reorder(res$sorted_groups[[t]], nna, reorder_pvalues)
  #})
  #groups<- as.data.frame(do.call(cbind, groups))
  groups <- fill_nas_reorder_dataframe(sorted_groups, nna, reorder_pvalues)
  
  if(covariate_type == "ordinal" & is.numeric(covariates)){
    #covariates <- apply(covariates, 2, function(covariate_i){
    #  fill_nas_reorder(covariate_i, nna, reorder_pvalues) #TODO
    #})
    covariates <- fill_nas_reorder_dataframe(covariates, nna, 1:nrow(covariates)) 
  }else if(is.factor(covariates)){
    covariates <- fill_nas_reorder(covariates, nna, 1:length(covariates))
  }

  #print(table(folds, groups))
  #print(t(res$weight_matrix[, ncol(res$weight_matrix):1]))
  ...
  ihw_obj <- new("ihwResult",
    ...
    m_groups = m_groups, #TODO as.integer(as.matrix(m_groups[[1]]))
    penalty = penalty,
...
ihw_internal <- function(sorted_groups, sorted_pvalues, alpha, lambdas,
                         ...
                         debug_flag = FALSE,
                         ...) {
  ntrees <- length(sorted_groups)
  folds_prespecified <- !is.null(sorted_folds)

  m <- length(sorted_pvalues)
  split_sorted_pvalues <- lapply(sorted_groups, function(sorted_groups_i) split(sorted_pvalues, sorted_groups_i))
  #m_groups_available <- sapply(split_sorted_pvalues, length) #TODO what about empty?
  m_groups_available <- lapply(split_sorted_pvalues , 
                               function(split_sorted_pvalues_i) sapply(split_sorted_pvalues_i, length)) #TODO what about empty?
  ...
  sorted_weights_tree <- matrix(NA, m, ntrees)
  fold_tree_lambdas <- matrix(NA, nfolds, ntrees)
  weight_matrix <- lapply(sorted_groups, function(sorted_groups_i) matrix(NA, nlevels(sorted_groups_i), nfolds))
  ...
  for(t in 1:ntrees){
    for (i in 1:nfolds) {
      ...
      if (!quiet) message(paste("Estimating weights for fold", i, "tree", t))
      # don't worry about inefficencies right now, they are only O(n) and sorting dominates this
      if (nfolds == 1) {
        filtered_sorted_groups <- sorted_groups[[t]]
        filtered_sorted_pvalues <- sorted_pvalues
        filtered_split_sorted_pvalues <- split(filtered_sorted_pvalues, filtered_sorted_groups)
        
        m_groups_holdout_fold_tree <- m_groups[[t]]
        m_groups_other_folds_tree <- m_groups[[t]]
      } else {
        filtered_sorted_groups <- sorted_groups[sorted_folds != i, t, drop = FALSE] #TODO does not make sense, if folds not prespecified
        ...
        if (!folds_prespecified) {
          ...
          m_groups_holdout_fold_tree <- (m_groups[[t]] - m_groups_available[[t]]) / nfolds + 
            m_groups_available[[t]] - sapply(filtered_split_sorted_pvalues, length)
          m_groups_other_folds_tree <- m_groups[[t]] - m_groups_holdout_fold_tree 
          ...
          m_groups_holdout_fold_tree <- m_groups[[t]][, i]
          m_groups_other_folds_tree <- rowSums(m_groups[[t]][, -i, drop = FALSE])
        }
      }
      ...
      if (distrib_estimator == "grenander") {
        res <- ihw_convex(filtered_split_sorted_pvalues, alpha,
                          m_groups_holdout_fold_tree, m_groups_other_folds_tree,
        ...
        res <- ihw_milp(filtered_split_sorted_pvalues, alpha, m_groups_holdout_fold_tree,
                        penalty = penalty, lambda = lambda, lp_solver = lp_solver, ...
        )
      } else {
      ...
      sorted_weights_tree[sorted_folds == i, t] <- res$ws[sorted_groups[[t]][sorted_folds == i]] #TODO + tree
      weight_matrix[[t]][, i] <- res$ws
      
      if (null_proportion) {
        pi0_est <- weighted_storey_pi0(sorted_pvalues[sorted_folds == i],
                                       sorted_weights_tree[sorted_folds == i, t],
                                       tau = null_proportion_level,
                                       m = sum(m_groups_holdout_fold_tree)
        )
        
        sorted_weights_tree[sorted_folds == i, t]  <- sorted_weights_tree[sorted_folds == i, t]  / pi0_est
        weight_matrix[[t]][, i] <- weight_matrix[[t]][, i] / pi0_est
      }
    }
  }

  sorted_weights <- rowMeans(sorted_weights_tree)
  sorted_weighted_pvalues <- mydiv(sorted_pvalues, sorted_weights)

  sorted_adj_p <- p.adjust(sorted_weighted_pvalues, method = adjustment_type, n = m) #sum(m_groups[[1]]) before
  rjs <- sum(sorted_adj_p <= alpha)

  lst <- list(
    lambda = lambda, fold_lambdas = fold_tree_lambdas, rjs = rjs, sorted_pvalues = sorted_pvalues,
...
ihw.DESeqResults <- function(deseq_res, filter = "baseMean",
  ...
  ihw_res <- ihw(deseq_res$pvalue, filter,
    alpha = alpha,
    adjustment_type = adjustment_type, ...
  )
...
ihw_convex <- function(split_sorted_pvalues, alpha, m_groups, m_groups_grenander,
  ...
  if (lambda < Inf) {
    if (penalty == "total variation") {
      ...
      constr_matrix <- rbind(
        cbind(constr_matrix, slam::simple_triplet_zero_matrix(nconstraints, nbins - 1, mode = "double")),
      ...
      absolute_val_constr_matrix_1 <- cbind(
        slam::simple_triplet_zero_matrix(nbins, nbins, mode = "double"),
        absolute_val_constr_matrix_1,
        -diag(nbins)
      )
      ...
      absolute_val_constr_matrix_2 <- cbind(
        slam::simple_triplet_zero_matrix(nbins, nbins, mode = "double"),
        absolute_val_constr_matrix_2,
        -diag(nbins)
      )
      ...
      constr_matrix <- rbind(
        cbind(constr_matrix, slam::simple_triplet_zero_matrix(nconstraints, nbins, mode = "double")),
        absolute_val_constr_matrix_1,
        absolute_val_constr_matrix_2
      )
  ...
  if (lp_solver == "gurobi") {
    ...
    model$A <- Matrix::sparseMatrix(
      i = constr_matrix$i, j = constr_matrix$j, x = constr_matrix$v,
    ...
    suppressMessages(res <- gurobi(model, params))
    sol <- res$x
    ...
    ts <- pmax(sol[(1:nbins) + nbins], 0)
    ws <- thresholds_to_weights(ts, m_groups) # \approx ts/sum(ts)*nbins
  } else if (lp_solver == "algorithm5") {
    slope.knots.vec <- lapply(grenander_list, function(x) x$slope.knots)
    x.knots.l.vec <- lapply(grenander_list, function(x) x$x.knots)
    y.knots.vec <- lapply(grenander_list, function(x) x$y.knots)

    # invert_subgradient <- function(slope.knots, x.knots.l, lambda) {
    #  if (lambda > slope.knots[1]) {
    #    return(c("l" = 0, "u" = 0))
    #  }
    #  else if (lambda <= 0) {
    #    return(c("l" = 1, "u" = 1))
    #  } else {
    #    idx <- base::findInterval(-lambda, -slope.knots) # , all.inside = TRUE
    #    l <- ifelse(slope.knots[idx] == lambda,
    #      x.knots.l[idx],
    #      x.knots.l[idx + 1]
    #    )
    #    u <- x.knots.l[idx + 1]
    ...
    # invert_subgradient_vec <- function(slope.knots.vec, x.knots.l.vec, lambda) {
    # slope.knots.vec <- lapply(b_set, function(x) x$slope.knots)
    #  x.knots.l.vec <- lapply(b_set, function(x) x$x.knots.l)
    # ts <- invert_subgradient_vecC(slope.knots.vec, x.knots.l.vec, lambda)

    #  ts <- mapply(function(slope.knots, x.knots.l) {
    #    invert_subgradient(slope.knots, x.knots.l, lambda)
    #  },slope.knots.vec, x.knots.l.vec)
    #  ts <- t(ts)
    #  ts <- as.data.frame(ts)
    #  colnames(ts) <- c("ts_L","ts_R")
    #  return(ts)
    # }

    # probability density function of grenander distribution
    grenander_pdf <- function(slope.knots, x.knots.l, x, tolerance = 1e-10) { # pdf
      if (x >= 1 | x < 0) {
        return(c(0, 0))
      } else if (x == 0) {
        return(c(0, slope.knots[1]))
      } else {
        # tolerance to handle numerical rounding and discontinuity points
        # idx1 <- base::findInterval(x, x.knots.l)
        # supdif_max <- slope.knots[idx1]
        # if(abs(x.knots.l[idx1]-x)< tolerance){
        #  supdif_min <- slope.knots[idx1+1]
        # }else{
        #  supdif_min <- supdif_max
        # }

        idx1 <- base::findInterval(x, x.knots.l - tolerance)
        idx1 <- max(1, idx1)
        idx2 <- base::findInterval(x, x.knots.l + tolerance)
        idx2 <- max(1, idx2)
        supdif_min <- slope.knots[idx1]
        supdif_max <- slope.knots[idx2]
        return(c(supdif_min, supdif_max))
      }
    }

    # cumulative density function of grenander distribution
    grenander_cdf <- function(x.knots.l, y.knots, x) {
      if (x >= 1) {
        return(1)
      } else if (x <= 0) {
        return(0)
        ...
        # browser()
        idx <- base::findInterval(x, x.knots.l)

        loc_L <- x.knots.l[idx]
        loc_U <- x.knots.l[idx + 1]
        t <- (x - loc_L) / (loc_U - loc_L)
    ...
    grenander_cdf_vec <- function(x.knots.l.vec, y.knots.vec, xs) {
      fs <- mapply(function(x.knots.l, y.knots, x) {
        grenander_cdf(x.knots.l, y.knots, x)
      }, x.knots.l.vec, y.knots.vec, xs)
      return(fs)
    }

    if (FALSE) {
      testthat::test_that("grenander_pdf & inverter", {
        mapply(function(x.knots.l, slope.knots) {
          x.knots.mid <- (x.knots.l + 1e-8)

          supdif_group <- t(sapply(x.knots.mid, function(x) grenander_pdf(slope.knots, x.knots.l, x)))

          b_set_group <- cbind(x.knots.l, slope.knots, supdif_group)
          test <- apply(b_set_group, 1, function(row) {
            # testthat::expect_true(row[["1"]] <= row[["slope.knots"]] & row[["slope.knots"]] <= row[["2"]])
            testthat::expect_equal(row[[3]], row[["slope.knots"]])
            # return(row[["1"]] == row[["slope.knots"]])
            # return(row[["1"]] <= row[["slope.knots"]] & row[["slope.knots"]] <= row[["1"]])
          })
          # if (!all(test)) browser()
          # lu <- invert_subgradient(slope.knots, x.knots.l, lambda)
          lu <- sapply(slope.knots, function(lambda) invert_subgradient(slope.knots, x.knots.l, lambda))
          lu <- t(lu)
          # browser()
          b_set_group <- cbind(b_set_group, lu)
          apply(b_set_group, 1, function(row) {
            testthat::expect_equal(row[[5]], row[["x.knots.l"]])
            # testthat::expect_equal(row[["u"]], row[["x.knots.u"]])
          })
        }, x.knots.l.vec, slope.knots.vec)
      })
    }


    if (adjustment_type == "bonferroni") {
      m_overall <- sum(m_groups + m_groups_grenander)
      k_alpha <- 1 * alpha / m_overall

      all_lambdas <- unlist(slope.knots.vec)
      all_lambdas <- all_lambdas[order(all_lambdas)]

      # while (1 < sum(sapply(b_set, nrow))) {
      while (length(all_lambdas) > 1) {
        # if (sum(sapply(b_set, nrow)) <= 2) browser()
        median_lambda <- all_lambdas[ceiling((length(all_lambdas) + 1) / 2)]

        # browser()
        ts <- invert_subgradient_vec(slope.knots.vec, x.knots.l.vec, median_lambda)
        ts_L <- ts[, "ts_L"]
        ts_R <- ts[, "ts_R"]

        wghtbudg_l <- sum(m_groups * ts_L) / (m * k_alpha)
        wghtbudg_u <- sum(m_groups * ts_R) / (m * k_alpha)

        # if(sum(sapply(b_set, nrow)) == 2) browser()
        if (wghtbudg_l > 1) {
          all_lambdas <- all_lambdas[all_lambdas > median_lambda]
        } else if (wghtbudg_u < 1) {
          all_lambdas <- all_lambdas[all_lambdas < median_lambda]
        } else {
          all_lambdas <- median_lambda
        }
      }

      lambda_opt <- all_lambdas

      ts <- invert_subgradient_vec(slope.knots.vec, x.knots.l.vec, lambda_opt)
      ts_L <- ts[, "ts_L"]
      ts_R <- ts[, "ts_R"]

      # if(TRUE){
      #  all_lambdas <- unlist(slope.knots.vec)
      #  ts2 <- lapply(all_lambdas, function(lambda) invert_subgradient_vec(slope.knots.vec, x.knots.l.vec, lambda))
      #  load("~/vectors.Rdata")
      #  ts3 <- sapply(ts2, function(ts){
      #    ts <- ts/k_alpha
      #    a <- ts[, "ts_L"] <= ws
      #    b <- ws <= ts[, "ts_R"]
      #    return(all(a) & all(b))
      ...
      c <- (wghtbudg_u - 1) / (wghtbudg_u - wghtbudg_l)
      ts_mixed <- c * ts_L + (1 - c) * ts_R
      ws <- ts_mixed / k_alpha
      # ws <- t_g *m_overall / alpha

      testthat::test_that("basic checks", {
        testthat::expect_gt(wghtbudg_u, 1)
        testthat::expect_lt(wghtbudg_l, 1)
      })

      testthat::test_that("KKT conditions stationarity", {
        # transform back
        ts_new <- ws * k_alpha

        # get super differential
        supdif <- mapply(
          function(slopeknots, xknotsl, t) {
            grenander_pdf(slopeknots, xknotsl, t)
          },
          slope.knots.vec, x.knots.l.vec, ts_new
        )
        supdif <- t(supdif)
        supdif <- as.data.frame(supdif)
        colnames(supdif) <- c("min_dif", "max_dif")

        supdif <- cbind(ts_new, supdif, ws)

        supdif_sub <- supdif[supdif$ws > 0, ]

        all_inside <- apply(supdif_sub, 1, function(row) { # TODO
          testthat::expect_true(row[["min_dif"]] <= lambda_opt & lambda_opt <= row[["max_dif"]])
        })

        if (!all(all_inside)) browser()
      })

      testthat::test_that("KKT conditions", {
        tolerance <- 1e-7
        max_slope.knot_g <- sapply(grenander_list, function(x) max(x$slope.knots))
        nu <- mapply(
          function(ws_g, m_g, max_slope.knot_g) {
            ifelse(ws_g > 0,
              0,
              m_g * k_alpha * (lambda_opt - max_slope.knot_g)
            )
          },
          ws, m_groups, max_slope.knot_g
        )

        # complementary slackness
        testthat::expect_equal(sum(ws * nu), 0, tolerance = tolerance)
        # dual feasibility
        for (ws_g in ws) testthat::expect_gte(ws_g, 0)
        # primal feasibility
        for (nu_g in nu) testthat::expect_gte(nu_g, 0)
        testthat::expect_equal(sum(ws * m_groups), m, tolerance = tolerance)

        # stationary
        # browser()
        test <- cbind(ts, ts_mixed) # TODO
        # all_inside <- apply(test, 1, function(row) {
        # testthat::expect_true(row[["ts_L"]] <= row[["ts_mixed"]] & row[["ts_mixed"]] <= row[["ts_R"]])
        #   testthat::expect_true(row[["l"]] - tolerance <= row[["ts"]] & row[["ts"]] <= row[["u"]] + tolerance)
        #  return(row[["ts_L"]] <= row[["ts_mixed"]] & row[["ts_mixed"]] <= row[["ts_R"]])
        # })
        # test[!all_inside, ]
        # if (!all(all_inside)) browser()
        ...
        # get super differential
        supdif <- mapply(
          function(slopeknots, xknotsl, ws_g) {
            grenander_pdf(slopeknots, xknotsl, ws_g * k_alpha)
          },
          slope.knots.vec, x.knots.l.vec, ws
        )

        supdif <- t(supdif)
        supdif <- as.data.frame(supdif)
        colnames(supdif) <- c("min_dif", "max_dif")
        supdif <- cbind(supdif, test)

        # browser()
        supdif$min_dif <- m_groups * k_alpha * (supdif$min_dif - lambda_opt) + nu
        supdif$max_dif <- m_groups * k_alpha * (supdif$max_dif - lambda_opt) + nu

        # all_inside <- apply(supdif, 1, function(row) {
        #  testthat::expect_true(row[["min_dif"]] <= 0 & 0 <= row[["max_dif"]])
        # testthat::expect_true(row[["min_dif"]] - tolerance <= 0 & 0 <= row[["max_dif"]] + tolerance)
        # })
        # if (!all(all_inside)) browser()
      })
    } else if (adjustment_type == "BH") {
      Fdr <- function(ts, Fs, m_groups) {
        num <- sum(ts * m_groups)
        denom <- sum(Fs * m_groups)
        ifelse(num == 0,
          0,
          num / denom
        )
      ...
      Fdr_linearized <- function(ts, Fs, m_groups, alpha) {
        num <- sum(ts * m_groups)
        denom <- sum(Fs * m_groups)
        num - alpha * denom
      }

      lagrange_balance <- function(lambda, slope.knots.vec, x.knots.l.vec, y.knots.vec, m_groups, alpha, linearized) {
        ts <- invert_subgradient_vec(slope.knots.vec, x.knots.l.vec, lambda)

        Fs_L <- grenander_cdf_vec(x.knots.l.vec, y.knots.vec, ts$ts_L)
        Fs_R <- grenander_cdf_vec(x.knots.l.vec, y.knots.vec, ts$ts_R)

        if (linearized) {
          Fdr_L <- Fdr_linearized(ts$ts_L, Fs_L, m_groups, alpha)
          Fdr_R <- Fdr_linearized(ts$ts_R, Fs_R, m_groups, alpha)
        } else {
          Fdr_L <- Fdr(ts$ts_L, Fs_L, m_groups)
          Fdr_R <- Fdr(ts$ts_R, Fs_R, m_groups)
        }

        return(c(Fdr_L = Fdr_L, Fdr_R = Fdr_R))
      }

      # find unique optimal lambda with binary search
      all_lambdas <- unlist(slope.knots.vec)
      all_lambdas <- all_lambdas[order(all_lambdas)]

      while (length(all_lambdas) > 1) {
        median_lambda <- all_lambdas[ceiling((length(all_lambdas) + 1) / 2)]
        non_lin_Fdr <- lagrange_balance(median_lambda, slope.knots.vec, x.knots.l.vec, y.knots.vec, m_groups, alpha, linearized = FALSE)

        if (non_lin_Fdr[1] > alpha) {
          all_lambdas <- all_lambdas[all_lambdas > median_lambda]
        } else if (non_lin_Fdr[2] < alpha) {
          all_lambdas <- all_lambdas[all_lambdas < median_lambda]
        } else {
          all_lambdas <- median_lambda
        }
      ...
      lin_Fdr <- lagrange_balance(lambda_opt, slope.knots.vec, x.knots.l.vec, y.knots.vec, m_groups, alpha, linearized = TRUE)
      tmp_comb <- lin_Fdr["Fdr_R"] / (lin_Fdr["Fdr_R"] - lin_Fdr["Fdr_L"])

      ts_mixed <- tmp_comb * ts_L + (1 - tmp_comb) * ts_R
      ts_mixed <- m * ts_mixed / sum(ts_mixed * m_groups)

      ws <- ts_mixed
      if (any(is.nan(ws) | is.infinite(ws))) {
        browser() #
        cbind(m_groups, ts_L, ts_R, ts_mixed) # TODO
        # TODO delete
        all_lambdas <- unlist(slope.knots.vec)
        all_lambdas <- all_lambdas[order(all_lambdas)]
        test <- sapply(all_lambdas, function(median_lambda) {
          non_lin_Fdr <- lagrange_balance(median_lambda, slope.knots.vec, x.knots.l.vec, y.knots.vec, m_groups, alpha, linearized = FALSE)
        })
        lambda_opt <- all_lambdas[test["Fdr_L", ] <= alpha & test["Fdr_R", ] >= alpha]
        ts <- invert_subgradient_vec(slope.knots.vec, x.knots.l.vec, lambda_opt)
        ts_L <- ts[, "ts_L"]
        ts_R <- ts[, "ts_R"]
  ...
  if (F) {
    testthat::test_that("a", {
      testthat::expect_equal(sum(ws * m_groups), m)
    })
    ...
    # probability density function of grenander distribution
    grenander_pdf <- function(slope.knots, x.knots.l, x, tolerance = 1e-20) { # pdf
      if (x >= 1 | x < 0) {
        return(c(0, 0))
      } else if (x == 0) {
        return(c(0, slope.knots[1]))
    ...
    obj_eval <- rowSums(supdif)
    print(paste("objective evaluated with", lp_solver, ":", obj_eval[1], ";", obj_eval[2]))
...
ihw_milp <- function(split_sorted_pvalues, alpha, m_groups, lambda = Inf, lp_solver = "gurobi",
  ...
  if (is.finite(lambda)) {
    ...
    ineq_matrix <- rbind(cbind(
      do.call(cbind, mapply(left_ineq_matrix_block, 1:nbins, p_lengths, pdiff_list, SIMPLIFY = FALSE)),
      diag(1, nbins)
    ))
    ...
    if (penalty == "total variation") {
      ...
      abs_constraint_matrix <- rbind(
        cbind(
          slam::simple_triplet_zero_matrix(nbins - 1, z_vars, mode = "double"),
      ...
      abs_constraint_matrix <- rbind(
        cbind(
          slam::simple_triplet_zero_matrix(nbins, z_vars, mode = "double"),
  ...
  if (lp_solver == "lpsymphony") {
    ...
    full_triplet <- Matrix::sparseMatrix(
      i = full_triplet$i, j = full_triplet$j, x = full_triplet$v,
    ...
    params <- list(
      NumericFocus = 3, FeasibilityTol = 10^(-9), ScaleFlag = 0, Quad = 1, IntFeasTol = 10^(-9),
